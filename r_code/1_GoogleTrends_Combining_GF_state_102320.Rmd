---
title: "Google Trends Cleaning - state level"
author: "Eli + Jackson Michaels"
date: "10/23/2020"
output: pdf_document
---
...
# function to impute missing data and combine across samples
```{r}
library(lme4)
library(dplyr)
library(tidyr)
...
# modified Pew code to automatically run on every name:
# create list of names:
names = c("AA", "BT", "GF")

# # States - each time we run, update with the person's name and search sampler will
# run the code for all the states under that person's name
starting_dir = '~/Box Sync/Research/Thu Projects/George Floyd Paper/Google/Data/State/'
# for loop to run on every name:
for (name in names)
{
  # modified Pew code to automatically run on every state:
  # create list of states:
  states = c("US-AL", "US-AK", "US-AZ", "US-AR", "US-CA", "US-CO", "US-CT", "US-DC", "US-DE", "US-FL", "US-GA", "US-HI", "US-ID", "US-IL", "US-IN", "US-IA", "US-KS", "US-KY", "US-LA", "US-ME", "US-MD", "US-MA", "US-MI", "US-MN", "US-MS", "US-MO", "US-MT", "US-NE", "US-NV", "US-NH", "US-NJ", "US-NM", "US-NY", "US-NC", "US-ND", "US-OH", "US-OK", "US-OR", "US-PA", "US-RI", "US-SC", "US-SD", "US-TN", "US-TX", "US-UT", "US-VT", "US-VA", "US-WA", "US-WV", "US-WI", "US-WY")
  
  # for loop to run on every state:
  for (state in states)
  {
       process_region(paste0(starting_dir, paste0(name, "/states")), state)
  }
}
